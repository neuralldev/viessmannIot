...
*/
    ...
    public function toHtml($_version = 'dashboard')
    {
        ...
        $replace = array_merge($replace, $this->getCommonReplaceValues());
        ...
        $replace = array_merge($replace, $this->getTemperatureReplaceValues());
        $replace = array_merge($replace, $this->getConsumptionReplaceValues());
        $replace = array_merge($replace, $this->getScheduleReplaceValues());
        $replace = array_merge($replace, $this->getProgramReplaceValues());
        $replace = array_merge($replace, $this->getMinMaxTemperatureReplaceValues());
    ...
    private function getCommonReplaceValues()
    {
        $replace = [];
        $cmds = [
            'refresh', 'isHeatingBurnerActive', 'isHeatingCompressorActive', 'currentError', 'outsideTemperature',
            'activeProgram', 'roomTemperature', 'secondaryCircuitTemperature', 'programTemperature',
            'hotWaterStorageTemperature', 'dhwTemperature', 'dhwGazConsumption', 'heatingGazConsumption',
            'dhwPowerConsumption', 'heatingPowerConsumption', 'refreshDate', 'heatingBurnerHours',
            'heatingBurnerHoursPerDay', 'heatingBurnerStarts', 'heatingBurnerStartsPerDay', 'heatingBurnerModulation',
            'heatingCompressorHours', 'heatingCompressorHoursPerDay', 'heatingCompressorStarts',
            'heatingCompressorStartsPerDay', 'slope', 'shift', 'pressureSupply', 'solarTemperature',
            'solarDhwTemperature', 'lastServiceDate', 'serviceInterval', 'monthSinceService', 'errors', 'activeMode',
            'activeDhwMode', 'comfortProgramTemperature', 'normalProgramTemperature', 'reducedProgramTemperature',
            'supplyProgramTemperature', 'boilerTemperature', 'boilerTemperatureMain', 'frostProtection', 'pumpStatus',
            'heatingSchedule', 'dhwSchedule', 'setHeatingSchedule', 'setDhwSchedule', 'heatingGazConsumptionDay',
            'dhwGazConsumptionDay', 'totalGazConsumptionDay', 'heatingGazConsumptionWeek', 'dhwGazConsumptionWeek',
            'totalGazConsumptionWeek', 'heatingGazConsumptionMonth', 'dhwGazConsumptionMonth', 'totalGazConsumptionMonth',
            'heatingGazConsumptionYear', 'dhwGazConsumptionYear', 'totalGazConsumptionYear', 'heatingPowerConsumptionDay',
            'heatingPowerConsumptionWeek', 'heatingPowerConsumptionMonth', 'heatingPowerConsumptionYear',
            'isOneTimeDhwCharge', 'isActivateComfortProgram', 'isActivateEcoProgram', 'isActivateLastSchedule',
            'isScheduleHolidayProgram', 'isScheduleHolidayAtHomeProgram', 'curve'
        ];
        ...
        foreach ($cmds as $cmd) {
            ...
            if (is_object($obj)) {
                ...
                $replace["#{$cmd}#"] = $cmd === 'errors' || $cmd === 'currentError' || $cmd === 'lastServiceDate' ? '' : 99;
    ...
    private function getTemperatureReplaceValues()
    {
        $replace = [];
        $replace["#range_temperature#"] = implode(',', array_map(function ($ot) {
            return "'$ot'";
        }, range(25, -20, -5)));
    ...
    private function getConsumptionReplaceValues()
    {
        $replace = [];
        ...
        $replace["#joursSemaine#"] = implode(',', array_map(function($j) use ($jours) {
            return "'" . $jours[$j] . "'";
        }, range($jour, $jour - 7, -1)));

        $replace["#semaines#"] = implode(',', array_map(function($i) use (&$semaine, &$maintenant) {
            $week = "'" . $semaine . "'";
            $maintenant -= 7 * 24 * 60 * 60;
            $semaine = date("W", $maintenant);
            return $week;
        }, range(0, substr_count($replace["#heatingGazConsumptionWeek#"], ",") + 1)));

        $replace["#moisS#"] = implode(',', array_map(function($i) use (&$mois, $libMois) {
            $moisS = "'" . $libMois[$mois] . "'";
            $mois = ($mois - 1 + 12) % 12;
            return $moisS;
        }, range(0, substr_count($replace["#heatingGazConsumptionMonth#"], ",") + 1)));

        $replace["#annees#"] = implode(',', array_map(function($i) use (&$annee) {
            return "'" . $annee-- . "'";
        }, range(0, substr_count($replace["#heatingGazConsumptionYear#"], ",") + 1)));

        return $replace;
    }

    private function getScheduleReplaceValues()
    {
        $replace = [];
        $replace["#idSetHeatingSchedule#"] = $this->getCmd(null, 'setHeatingSchedule')->getId();
        $replace["#idSetDhwSchedule#"] = $this->getCmd(null, 'setDhwSchedule')->getId();

        return $replace;
    }

    private function getProgramReplaceValues()
    {
        $replace = [];
        $replace["#range_temp#"] = implode(',', array_map(function ($ot) {
            return "'$ot'";
        }, range(25, -20, -5)));
    ...
    private function getMinMaxTemperatureReplaceValues()
    {
        $replace = [];
        ...
        $maintenant = time();
        $jour = date("N", $maintenant) - 1;
        $replace["#joursMinMax#"] = implode(',', array_map(function($j) use ($jours) {
            return "'" . $jours[$j] . "'";
        }, range($jour, $jour - 7, -1)));
